...
extern "C" {
WINPATHCCHAPI
BOOL APIENTRY PathIsUNCEx(_In_ PCWSTR pszPath, _Outptr_opt_ PCWSTR *ppszServer);

WINPATHCCHAPI
BOOL APIENTRY PathCchIsRoot(_In_opt_ PCWSTR pszPath);
...
HRESULT
APIENTRY
PathCchRemoveBackslash(_Inout_updates_(cchPath) PWSTR pszPath, _In_ size_t cchPath);

WINPATHCCHAPI
HRESULT
APIENTRY
PathCchSkipRoot(_In_ PCWSTR pszPath, _Outptr_ PCWSTR *ppszRootEnd);

WINPATHCCHAPI
HRESULT
APIENTRY
PathCchStripToRoot(_Inout_updates_(_Inexpressible_(cchPath)) PWSTR pszPath, _In_ size_t cchPath);

WINPATHCCHAPI
HRESULT
APIENTRY
PathCchRemoveFileSpec(_Inout_updates_(_Inexpressible_(cchPath)) PWSTR pszPath, _In_ size_t cchPath);
...
HRESULT
APIENTRY
PathCchAppend(_Inout_updates_(cchPath) PWSTR pszPath, _In_ size_t cchPath, _In_opt_ PCWSTR pszMore);

WINPATHCCHAPI
HRESULT
APIENTRY
PathCchStripPrefix(_Inout_updates_(cchPath) PWSTR pszPath, _In_ size_t cchPath);
...
HRESULT
APIENTRY
PathAllocCanonicalize(_In_ PCWSTR pszPathIn, _In_ ULONG dwFlags, _Outptr_ PWSTR *ppszPathOut);
...
#define __DEFINE_THUNK(_MODULE, _SIZE, _RETURN_, _CONVENTION_, _FUNCTION, ...)                     \
    __if_exists(YY::Thunks::TopFix::_FUNCTION)                                                     \
    {                                                                                              \
        decltype(YY::Thunks::TopFix::_FUNCTION) *_CRT_CONCATENATE(FunctionInclude, _FUNCTION) =    \
            &YY::Thunks::TopFix::_FUNCTION;                                                        \
    }                                                                                              \
    __if_not_exists(YY::Thunks::TopFix::_FUNCTION)                                                 \
    {                                                                                              \
        decltype(::_FUNCTION) *_CRT_CONCATENATE(FunctionInclude, _FUNCTION) = &::_FUNCTION;        \
    }                                                                                              \
...
struct string_less
{
    ...
    bool operator()(LPCSTR _Left, LPCSTR _Right) const { return strcmp(_Left, _Right) < 0; }
};
...
ErrorInfo
    .Format(L"%ws 模块：%hs 函数名称：%hs 没有被Thunks成功！", szType, szDllName, szImportName);
...
TEST_METHOD(符号Thunks检测)
{
    ...
    do
    {
        ...
        auto pImportDescriptor =
            (IMAGE_IMPORT_DESCRIPTOR *)((char *)&__ImageBase + pImort->VirtualAddress);
        ...
        for (; pImportDescriptor->Name; ++pImportDescriptor)
        {
            ...
            auto pThunkData =
                (IMAGE_THUNK_DATA *)((char *)&__ImageBase + pImportDescriptor->OriginalFirstThunk);

            for (; pThunkData->u1.AddressOfData; ++pThunkData)
            {
                // 这是序数导入
                if (pThunkData->u1.AddressOfData >>
                    ((sizeof(pThunkData->u1.AddressOfData) * 8) - 1))
                    continue;

                auto pImportByName =
                    (PIMAGE_IMPORT_BY_NAME)((char *)&__ImageBase + pThunkData->u1.ForwarderString);
                ...
                if (Names.find(pImportByName->Name) != Names.end())
                {
                    ReportModule(
                        ...
                        L"IMAGE_DIRECTORY_ENTRY_IMPORT");
                }
            }
        }
    } while (false);
    ...
    do
    {
        ...
        auto pDelayLoadDescriptor =
            (IMAGE_DELAYLOAD_DESCRIPTOR *)((char *)&__ImageBase + pImort->VirtualAddress);

        for (; pDelayLoadDescriptor->DllNameRVA; ++pDelayLoadDescriptor)
        {
            auto pThunkData =
                (IMAGE_THUNK_DATA
                     *)((char *)&__ImageBase + pDelayLoadDescriptor->ImportNameTableRVA);
            ...
            for (; pThunkData->u1.AddressOfData; ++pThunkData)
            {
                ...
                if (Names.find(pImportByName->Name) != Names.end())
                {
                    ReportModule(
                        pImportByName->Name,
                        pDelayLoadDescriptor->DllNameRVA,
                        L"IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT");
                }
            }
        }
