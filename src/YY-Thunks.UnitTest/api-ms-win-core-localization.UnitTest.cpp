...
TEST_CLASS(GetThreadPreferredUILanguages){public : GetThreadPreferredUILanguages(){
...
TEST_METHOD(无效参数验证)
{
    ...
    auto bRet = ::GetThreadPreferredUILanguages(
        MUI_LANGUAGE_NAME, nullptr, szLanguagesBuffer, &cchLanguagesBuffer);
    Assert::IsFalse(bRet);

    cchLanguagesBuffer = _countof(szLanguagesBuffer);
    // pwszLanguagesBuffer 为nullptr 时， pcchLanguagesBuffer 必须为 0
    bRet = ::GetThreadPreferredUILanguages(
        MUI_LANGUAGE_NAME, &ulNumLanguages, nullptr, &cchLanguagesBuffer);
    Assert::IsFalse(bRet);

    // pcchLanguagesBuffer不能为 nullptr
    bRet = ::GetThreadPreferredUILanguages(
        MUI_LANGUAGE_NAME, &ulNumLanguages, szLanguagesBuffer, nullptr);
    Assert::IsFalse(bRet);
    ...
    cchLanguagesBuffer = _countof(szLanguagesBuffer);
    bRet = ::GetThreadPreferredUILanguages(
        MUI_MERGE_SYSTEM_FALLBACK | MUI_THREAD_LANGUAGES,
        &ulNumLanguages,
        szLanguagesBuffer,
        &cchLanguagesBuffer);
    Assert::IsFalse(bRet);
    ...
    cchLanguagesBuffer = _countof(szLanguagesBuffer);
    bRet = ::GetThreadPreferredUILanguages(
        MUI_CONSOLE_FILTER, &ulNumLanguages, szLanguagesBuffer, &cchLanguagesBuffer);
    Assert::IsFalse(bRet);

    // 不允许使用MUI_COMPLEX_SCRIPT_FILTER
    cchLanguagesBuffer = _countof(szLanguagesBuffer);
    bRet = ::GetThreadPreferredUILanguages(
        MUI_COMPLEX_SCRIPT_FILTER, &ulNumLanguages, szLanguagesBuffer, &cchLanguagesBuffer);
    Assert::IsFalse(bRet);
}
...
TEST_METHOD(MUI_LANGUAGE_ID验证)
{
    ...
    // pulNumLanguages 不能为 nullptr
    auto bRet = ::GetThreadPreferredUILanguages(
        MUI_LANGUAGE_ID | MUI_THREAD_LANGUAGES,
        &ulNumLanguages,
        szLanguagesBuffer,
        &cchLanguagesBuffer);
    Assert::IsTrue(bRet);
...
TEST_METHOD(MUI_LANGUAGE_NAME验证)
{
    ...
    // 默认为 MUI_LANGUAGE_NAME
    {
        ...
        // pulNumLanguages 不能为 nullptr
        auto bRet = ::GetThreadPreferredUILanguages(
            MUI_THREAD_LANGUAGES, &ulNumLanguages, szLanguagesBuffer, &cchLanguagesBuffer);
        Assert::IsTrue(bRet);
...
TEST_METHOD(Languages消重验证)
{
...

    {
        ...
        // pulNumLanguages 不能为 nullptr
        auto bRet = ::GetThreadPreferredUILanguages(
            MUI_LANGUAGE_ID | MUI_MERGE_USER_FALLBACK,
            &ulNumLanguages,
            szLanguagesBuffer,
            &cchLanguagesBuffer);
        Assert::IsTrue(bRet);
...
TEST_CLASS(ResolveLocaleName){
    public : ResolveLocaleName(){YY::Thunks::aways_null_wp_get_ResolveLocaleName = true;
}
