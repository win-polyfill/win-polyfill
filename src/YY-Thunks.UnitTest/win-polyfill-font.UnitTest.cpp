...
struct ITaskbarList3 : ITaskbarList2
{
    ...
    SetProgressValue(HWND hwnd, ULONGLONG completed, ULONGLONG total) = 0;
    ...
    virtual HRESULT STDMETHODCALLTYPE
    ThumbBarAddButtons(HWND hwnd, UINT count, THUMBBUTTON buttons) = 0;
    virtual HRESULT STDMETHODCALLTYPE
    ThumbBarUpdateButtons(HWND hwnd, UINT count, THUMBBUTTON *buttons) = 0;
    virtual HRESULT STDMETHODCALLTYPE ThumbBarSetImageList(HWND hwnd, HIMAGELIST imglist) = 0;
    virtual HRESULT STDMETHODCALLTYPE
    SetOverlayIcon(HWND hwnd, HICON icon, LPCWSTR description) = 0;
...
struct IApplicationDocumentLists : public IUnknown
{
  public:
    virtual HRESULT STDMETHODCALLTYPE SetAppID(LPCWSTR pszAppID) = 0;
    virtual HRESULT STDMETHODCALLTYPE
    GetList(APPDOCLISTTYPE listtype, UINT cItemsDesired, REFIID riid, void **ppv) = 0;
};
#endif
...
{
    IShellLink *psl; // An IShellLink instance that when launched opens a recently used item in the
                     // specified application. This link is not added to the recent docs folder, but
                     // will be added to the specified application's destination list.
    PCWSTR pszAppID; // The id of the application that should be associated with this recent doc.
} SHARDAPPIDINFOLINK;

#endif // !defined(__MINGW64_VERSION_MAJOR) || !defined(__MINGW64_VERSION_MINOR) ||
       // __MINGW64_VERSION_MAJOR * 100 + __MINGW64_VERSION_MINOR < 301
