...
typedef struct __declspec(align(8)) _WP_ADDRESS_WAIT_BLOCK
{
    ...
    //因为Windows 8以及更高版本才支持 ZwWaitForAlertByThreadId，所以我们直接把 ThreadId
    //砍掉了，反正没鸟用 ULONG_PTR            ThreadId;
...
static HANDLE __fastcall GetGlobalKeyedEventHandle()
{
    ...
    // Windows XP等平台则 使用系统自身的 CritSecOutOfMemoryEvent，Vista或者更高平台 我们直接返回
    // nullptr 即可。
    if (NtCurrentTeb()->ProcessEnvironmentBlock->OSMajorVersion < 6)
    {
        if (_GlobalKeyedEventHandle == nullptr)
        {
            ...
            if (InterlockedCompareExchange(
                    (size_t *)&_GlobalKeyedEventHandle, (size_t)KeyedEventHandle, (size_t) nullptr))
            {
...
static void __fastcall RtlpWakeSRWLock(SRWLOCK *SRWLock, size_t Status)
{
    ...
    for (;;)
    {
        ...
        {
            auto NewStatus = InterlockedCompareExchange(
                (volatile LONG *)SRWLock, Status & ~WP_SRWLOCK_Waking, Status);
            if (NewStatus == Status)
                return;
...
static void __fastcall RtlpOptimizeSRWLockList(SRWLOCK *SRWLock, size_t Status)
{
    for (;;)
    {
        if (Status & WP_SRWLOCK_Locked)
        {
            ...
            auto CurrentStatus = InterlockedCompareExchange(
                (volatile size_t *)SRWLock, Status - WP_SRWLOCK_Waking, Status);
...
//将等待块插入 SRWLock 中
static BOOL __fastcall RtlpQueueWaitBlockToSRWLock(
    WP_CV_WAIT_BLOCK *pBolck,
    PSRWLOCK SRWLock,
    DWORD SRWLockMark)
{
    for (;;)
...
static void __fastcall RtlpWakeConditionVariable(
    PCONDITION_VARIABLE ConditionVariable,
    size_t ConditionVariableStatus,
    size_t WakeCount)
{
    auto GlobalKeyedEventHandle = GetGlobalKeyedEventHandle();
    auto pNtReleaseKeyedEvent = wp_get_NtReleaseKeyedEvent();
    ...
    for (;;)
    {
        ...
        {
            const auto LastStatus = InterlockedCompareExchange(
                (volatile size_t *)ConditionVariable, size_t(pWaitBlock), ConditionVariableStatus);
        ...
        else
        {
            ...
            {
                const auto LastStatus = InterlockedCompareExchange(
                    (volatile size_t *)ConditionVariable,
                    size_t(pWaitBlock),
            ...
            {
                const auto LastStatus = InterlockedCompareExchange(
                    (volatile size_t *)ConditionVariable, 0, ConditionVariableStatus);
    ...
    for (; pWake;)
    {
        ...
        {
            if (pWake->SRWLock == nullptr ||
                RtlpQueueWaitBlockToSRWLock(pWake, pWake->SRWLock, (pWake->flag >> 2) & 0x1) ==
                    FALSE)
            {
...
static void __fastcall RtlpOptimizeConditionVariableWaitList(
    PCONDITION_VARIABLE ConditionVariable,
    size_t ConditionVariableStatus)
{
    for (;;)
...
static BOOL __fastcall RtlpWakeSingle(
    PCONDITION_VARIABLE ConditionVariable,
    WP_CV_WAIT_BLOCK *pBlock)
{
    ...
    for (; Current && (Current & 0x7) != 0x7;)
    {
        if (Current & 0x8)
        {
            const auto Last = InterlockedCompareExchange(
                (volatile size_t *)ConditionVariable, Current | 0x7, Current);
        ...
        else
        {
            ...
            auto Last =
                InterlockedCompareExchange((volatile size_t *)ConditionVariable, New, Current);
                    ...
                    for (; pWaitBlock;)
                    {
                        if (pWaitBlock == pBlock)
                        {
                            ...
                            else
                            {
                                ...
                                Last = InterlockedCompareExchange(
                                    (volatile size_t *)ConditionVariable, New, Current);
...
static size_t __fastcall RtlpRunOnceWaitForInit(size_t Current, LPINIT_ONCE lpInitOnce)
{
    ...
    _Outptr_opt_result_maybenull_ LPVOID *lpContext)
{
    ...
    for (;;)
    {
        ...
        {
            const auto Last =
                InterlockedCompareExchange((volatile size_t *)lpInitOnce, New, Current);
            if (Last == Current)
        ...
        else
        {
            ...
            if (Current != (RTL_RUN_ONCE_CHECK_ONLY | RTL_RUN_ONCE_ASYNC))
            {
                if (lpContext)
                    *lpContext =
                        (LPVOID)(Current & ~size_t(RTL_RUN_ONCE_CHECK_ONLY | RTL_RUN_ONCE_ASYNC));
...
static void __fastcall RtlpRunOnceWakeAll(size_t *pWake)
{
    ...
    for (auto WakeAddress =
             (LPVOID)(*pWake & ~size_t(RTL_RUN_ONCE_CHECK_ONLY | RTL_RUN_ONCE_ASYNC));
         WakeAddress;)
    {
        //防止地址无效，我们先保存下
    ...
    _In_opt_ LPVOID lpContext)
{
    ...
    */
    ...
    if (lpContext && ((dwNewFlags & RTL_RUN_ONCE_ASYNC) == 0 ||
                      (size_t(lpContext) & (RTL_RUN_ONCE_CHECK_ONLY | RTL_RUN_ONCE_ASYNC))))
    {
        return STATUS_INVALID_PARAMETER_3;
    }

    auto Current = *(volatile size_t *)lpInitOnce;
    auto New = (size_t(lpContext) & ~size_t(RTL_RUN_ONCE_CHECK_ONLY | RTL_RUN_ONCE_ASYNC)) |
               (dwNewFlags & RTL_RUN_ONCE_ASYNC);
...
static void __fastcall RtlpOptimizeWaitOnAddressWaitList(volatile ULONG_PTR *ppFirstBlock)
{
    ...
    for (;;)
    {
        ...
        const auto Last = InterlockedCompareExchange(
            ppFirstBlock, (Current & 1) == 0 ? size_t(pBlock) : 0, Current);
...
static NTSTATUS __fastcall RtlpWaitOnAddressWithTimeout(
    WP_ADDRESS_WAIT_BLOCK *pWaitBlock,
        ...
        else
        {
            ...
            if (Last == Current)
            {
                ...
                for (;;)
                {
                    const auto Last = InterlockedCompareExchange(
                        ppFirstBlock,
                        (Current & 1) == 0 ? size_t(WP_ADDRESS_GET_BLOCK(Current)) : 0,
                        Current);
...
static void __fastcall WP_RtlpWakeByAddress(
    DWORD dwReserved /*用于平衡栈需要，利于编译器优化成jmp*/,
    BOOL bWakeAll,
    LPVOID Address)
#else
...
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, InitOnceInitialize, _Out_ PINIT_ONCE InitOnce)
{
    ...
    _Outptr_opt_result_maybenull_ LPVOID *Context)
{
    ...
    //别问我为什么不设置LastError……，Windows 7的这个函数也没设置
    // LastError，既然这样，我们都偷懒下吧，行为跟微软保持一致。
    ...
    _In_ DWORD dwDesiredAccess)
{
    ...
    return CreateWaitableTimerW(
        lpTimerAttributes, dwFlags & CREATE_WAITABLE_TIMER_MANUAL_RESET, lpTimerName);
}
#endif
...
// Windows Vista [desktop apps | UWP apps]
// Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, InitializeSRWLock, _Out_ PSRWLOCK SRWLock)
{
...
// Windows Vista [desktop apps | UWP apps]
// Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, AcquireSRWLockExclusive, _Inout_ PSRWLOCK SRWLock)
{
    ...
    for (;;)
    {
        ...
        if (WP_SRWLOCK_Locked & SRWLockOld)
        {
            ...
            */
            ...
            if (WP_SRWLOCK_Waiting & SRWLockOld)
            {
                ...
                SRWLockNew = (size_t)(&StackWaitBlock) | (SRWLockOld & WP_SRWLOCK_MultipleShared) |
                             WP_SRWLOCK_Waking | WP_SRWLOCK_Waiting | WP_SRWLOCK_Locked;
            ...
            else
            {
                ...
                SRWLockNew =
                    StackWaitBlock.shareCount > 1
                        ? (size_t)(&StackWaitBlock) | WP_SRWLOCK_MultipleShared |
                              WP_SRWLOCK_Waiting | WP_SRWLOCK_Locked
                        : (size_t)(&StackWaitBlock) | WP_SRWLOCK_Waiting | WP_SRWLOCK_Locked;
            }

            if (InterlockedCompareExchange((volatile size_t *)SRWLock, SRWLockNew, SRWLockOld) !=
                SRWLockOld)
            {
        ...
        else
        {
            //尝试获取锁的所有权
            if (InterlockedCompareExchange(
                    (volatile size_t *)SRWLock, SRWLockOld | WP_SRWLOCK_Locked, SRWLockOld) ==
                SRWLockOld)
            {
                //成功加锁
                return;
...
__DEFINE_THUNK(kernel32, 4, BOOLEAN, WINAPI, TryAcquireSRWLockExclusive, _Inout_ PSRWLOCK SRWLock)
{
...
// Windows Vista [desktop apps | UWP apps]
// Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, ReleaseSRWLockExclusive, _Inout_ PSRWLOCK SRWLock)
{
    ...
    if ((OldSRWLock & WP_SRWLOCK_Waiting) && (OldSRWLock & WP_SRWLOCK_Waking) == 0)
    {
        ...
        auto CurrentSRWLock =
            InterlockedCompareExchange((volatile size_t *)SRWLock, NewSRWLock, OldSRWLock);
...
// Windows Vista [desktop apps | UWP apps]
// Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, AcquireSRWLockShared, _Inout_ PSRWLOCK SRWLock)
{
    ...
    {
        if ((OldSRWLock & WP_SRWLOCK_Locked) &&
            ((OldSRWLock & WP_SRWLOCK_Waiting) || WP_SRWLOCK_GET_BLOCK(OldSRWLock) == nullptr))
        {
            // if ( RtlpWaitCouldDeadlock() )
            //    ZwTerminateProcess((HANDLE)0xFFFFFFFF, 0xC000004B);
            ...
            if (OldSRWLock & WP_SRWLOCK_Waiting)
            {
                ...
                NewSRWLock = size_t(&StackWaitBlock) | (OldSRWLock & WP_SRWLOCK_MultipleShared) |
                             (WP_SRWLOCK_Waking | WP_SRWLOCK_Waiting | WP_SRWLOCK_Locked);
            ...
            if (InterlockedCompareExchange((volatile size_t *)SRWLock, NewSRWLock, OldSRWLock) ==
                OldSRWLock)
            {
                //更新成功！
                ...
                if (InterlockedBitTestAndReset((volatile LONG *)&StackWaitBlock.flag, 1))
                {
                    pNtWaitForKeyedEvent(
                        GlobalKeyedEventHandle, (PVOID)&StackWaitBlock, 0, nullptr);
                }
...
// Windows 7 [desktop apps | UWP apps]
// Windows Server 2008 R2 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, BOOLEAN, WINAPI, TryAcquireSRWLockShared, _Inout_ PSRWLOCK SRWLock)
{
...
// Windows Vista [desktop apps | UWP apps]
// Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, ReleaseSRWLockShared, _Inout_ PSRWLOCK SRWLock)
{
    ...
    for (;;)
    {
        if (OldSRWLock & WP_SRWLOCK_Waiting)
        {
            ...
            for (;;)
            {
                ...
                {
                    LastSRWLock = InterlockedCompareExchange(
                        (volatile size_t *)SRWLock, NewSRWLock, OldSRWLock);
        ...
        {
            auto NewSRWLock =
                size_t(WP_SRWLOCK_GET_BLOCK(OldSRWLock)) <= 0x10 ? 0 : OldSRWLock - 0x10;
    ...
    _In_ DWORD dwMilliseconds)
{
    ...
    for (;;)
    {
        ...
        auto LastConditionVariable = InterlockedCompareExchange(
            (volatile size_t *)ConditionVariable, NewConditionVariable, OldConditionVariable);
    ...
    if (InterlockedBitTestAndReset((volatile LONG *)&StackWaitBlock.flag, 1))
    {
        ...
        Status = pNtWaitForKeyedEvent(
            GlobalKeyedEventHandle,
            (PVOID)&StackWaitBlock,
            0,
            internal::BaseFormatTimeOut(&TimeOut, dwMilliseconds));

        if (Status == STATUS_TIMEOUT &&
            internal::RtlpWakeSingle(ConditionVariable, &StackWaitBlock) == FALSE)
        {
            pNtWaitForKeyedEvent(GlobalKeyedEventHandle, (PVOID)&StackWaitBlock, 0, nullptr);
            Status = 0;
        }
    ...
    _Inout_ PCONDITION_VARIABLE ConditionVariable)
{
    ...
    for (; Current; Current = Last)
    {
        if (Current & 0x8)
        {
            ...
            Last = InterlockedCompareExchange(
                (volatile size_t *)ConditionVariable, Current + 1, Current);
            if (Last == Current)
                return;
        }
        else
        {
            Last = InterlockedCompareExchange(
                (volatile size_t *)ConditionVariable, Current | 0x8, Current);
            if (Last == Current)
            {
    ...
    _In_ LONG lSpinCount)
{
    ...
    pYYBarrier->dwNumProcessors =
        ((TEB *)NtCurrentTeb())->ProcessEnvironmentBlock->NumberOfProcessors;
    ...
    _In_ DWORD dwFlags)
{
    ...
    if (dwFlags &
        ~(SYNCHRONIZATION_BARRIER_FLAGS_SPIN_ONLY | SYNCHRONIZATION_BARRIER_FLAGS_BLOCK_ONLY |
          SYNCHRONIZATION_BARRIER_FLAGS_NO_DELETE))
    {
        internal::RaiseStatus(STATUS_INVALID_PARAMETER);
    }

    DWORD dwRtlBarrierFlags = dwFlags & (SYNCHRONIZATION_BARRIER_FLAGS_SPIN_ONLY |
    ...
    if (pYYBarrier->dwSpinCount &&
        ((dwRemainderThreads < pYYBarrier->dwNumProcessors &&
          (dwRtlBarrierFlags & SYNCHRONIZATION_BARRIER_FLAGS_BLOCK_ONLY) == 0) ||
         (dwRtlBarrierFlags & SYNCHRONIZATION_BARRIER_FLAGS_SPIN_ONLY)))
    {
        ...
        for (;;)
        {
            ...
            //指定 SYNCHRONIZATION_BARRIER_FLAGS_SPIN_ONLY
            //标识时会始终自旋，所以没有此标识时我们无需更新当前自旋次数
    ...
    _Inout_ LPSYNCHRONIZATION_BARRIER lpBarrier)
{
    ...
    //微软文档说的，这个函数总是返回 TRUE，但是微软Windows 8的实现略有问题，直接转发了
    // RtlDeleteBarrier，返回值并不正确
    return TRUE;
}
#endif
    ...
    _In_opt_ DWORD dwMilliseconds)
{
...
#if _WIN64
        // 64位自身能保证操作的原子性
        bSame = *(volatile unsigned long long *)Address ==
                *(volatile unsigned long long *)CompareAddress;
#else
        bSame = InterlockedCompareExchange64((volatile long long *)Address, 0, 0) ==
                *(volatile long long *)CompareAddress;
#endif
        break;
    }
    ...
    auto Status = internal::RtlpWaitOnAddressWithTimeout(
        &WaitBlock, internal::BaseFormatTimeOut(&TimeOut, dwMilliseconds));
...
// Windows 8 [desktop apps | UWP apps]
// Windows Server 2012 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, WakeByAddressSingle, _In_ PVOID Address)
{
...
// Windows 8 [desktop apps | UWP apps]
// Windows Server 2012 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, WakeByAddressAll, _In_ PVOID Address)
{
