...
static char *volatile *GetPageVirtualProtect()
{
    ...
    __declspec(allocate(".YYThr$AAB")) static void *RunUninitPageVirtualProtect =
        reinterpret_cast<void *>(&Fallback::UninitPageVirtualProtect);
...
static void __cdecl UninitPageVirtualProtect()
{
    auto pOrgPageVirtualProtect = (char *)InterlockedExchangePointer(
        (PVOID volatile *)GetPageVirtualProtect(), INVALID_HANDLE_VALUE);
...
__DEFINE_THUNK(kernel32, 0, DWORD, WINAPI, GetCurrentProcessorNumber, VOID)
{
...
// 感谢 过客 提供
__DEFINE_THUNK(kernel32, 4, DWORD, WINAPI, GetThreadId, _In_ HANDLE Thread)
{
...
// Windows Vista [desktop apps | UWP apps]
// Windows Server 2003 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, DWORD, WINAPI, GetProcessIdOfThread, _In_ HANDLE Thread)
{
...
__DEFINE_THUNK(kernel32, 0, VOID, WINAPI, FlushProcessWriteBuffers, VOID)
{
    ...
    */
    ...
    for (;;)
    {
        auto pOrgPageVirtualProtect = (char *)InterlockedCompareExchangePointer(
            (PVOID volatile *)Fallback::GetPageVirtualProtect(), nullptr, nullptr);
        ...
        else if (pOrgPageVirtualProtect == nullptr)
        {
            ...
            auto pPageVirtualProtectTmp =
                (char *)VirtualAlloc(NULL, 0x1000, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

            if (pPageVirtualProtectTmp == nullptr)
            {
                // 内存申请失败，直接设置为 -1，并且什么也不做
                InterlockedCompareExchangePointer(
                    (PVOID volatile *)Fallback::GetPageVirtualProtect(),
            ...
            *pPageVirtualProtectTmp = 1;
            ...
            pOrgPageVirtualProtect = (char *)InterlockedCompareExchangePointer(
                (PVOID volatile *)Fallback::GetPageVirtualProtect(),
                pPageVirtualProtectTmp,
                nullptr);
        ...
        // Note that the read of *m_pPageVirtualProtect is very important, as it makes it extremely
        // likely that this memory will be in the working set when we call VirtualProtect (see
        // comments below).
        if (*pOrgPageVirtualProtect == 1)
        {
            //
            // VirtualProtect simulates FlushProcessWriteBuffers because it happens to send an
            // inter-processor interrupt to all CPUs, and inter-processor interrupts happen to cause
            // the CPU's store buffers to be flushed.
            //
            // Unfortunately, VirtualProtect only does this if the page whose status is being
            // changed is in the process' working set (otherwise there's no need to tell the other
            // CPUs that anything has changed).
            //
            // One way to do this is to lock the page into the process' working set. Unfortunately,
            // it can fail if there are already too many locked pages.
            //
            // We could increase the process' working set limit, using SetProcessWorkingSet, but
            // that would be a) intrusive (the process may have its own idea of what the limit
            // should be), and b) race-prone (another thread may be trying to adjust the limit, to a
            // different value, at the same time).
            //
            // We could stop using *m_pPageVirtualProtect as the page we fiddle with, and instead
            // use a page we know is already locked into the working set. There's no way to
            // enumerate such pages, so it'd have to be a well-known fixed location that we know is
            // always locked, and that can have its protection fiddled with without consequence.  We
            // know of no such location, and if we did it would undoubtedly be some internal Windows
            // data structure that would be subject to changes in the way its memory is handled at
            // any time.
            //
            // The VirtualProtect trick has worked for many years in the CLR, without the call to
            // VirtualLock, without apparent problems. Part of the reason is because of the
            // equivalent of the check of *m_pPageVirtualProtect above.
            //
            DWORD oldProtect;

            // We have it on good authority from the kernel team that, although VirtualProtect is
            // repeatedly called with the same protection (PAGE_READONLY), the OS will not optimize
            // out the flush buffers as a result.
...
__DEFINE_THUNK(
    ...
    _When_(lpAttributeList == nullptr, _Out_) _When_(lpAttributeList != nullptr, _Inout_)
        PSIZE_T lpSize)
{
    ...
    do
    {
        ...
        const auto cbSizeNeed = sizeof(PROC_THREAD_ATTRIBUTE_LIST) +
    ...
    _In_opt_ PSIZE_T lpReturnSize)
{
    ...
    {
        return pUpdateProcThreadAttribute(
