...
__DEFINE_THUNK(
    kernel32,
    4,
    _Success_(return != TIME_ZONE_ID_INVALID) DWORD,
    WINAPI,
    ...
    _Out_ PDYNAMIC_TIME_ZONE_INFORMATION pTimeZoneInformation)
{
    ...
    do
    {
        ...
        if (RegOpenKeyExW(
                HKEY_LOCAL_MACHINE,
                LR"(SOFTWARE\Microsoft\Windows NT\CurrentVersion\Time Zones)",
                0,
                KEY_READ,
                &hTimeZoneRootKey) != ERROR_SUCCESS)
        {
            break;
        }
        ...
        for (unsigned i = 0;
             RegEnumKeyW(hTimeZoneRootKey, i, szTimeZoneKeyName, _countof(szTimeZoneKeyName)) ==
             ERROR_SUCCESS;
             ++i)
        {
            HKEY hTimeZoneKey;
            if (RegOpenKeyExW(hTimeZoneRootKey, szTimeZoneKeyName, 0, KEY_READ, &hTimeZoneKey) !=
                ERROR_SUCCESS)
                continue;
            ...
            auto lStatus = RegQueryValueExW(
                hTimeZoneKey, L"Std", nullptr, nullptr, (LPBYTE)StandardName, &cdStandardName);
