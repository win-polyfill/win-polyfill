...
{
    static const _TP_CLEANUP_GROUP_FUNCS TppWorkpCleanupGroupMemberVFuncs = {
        &TppWorkpFree, &TppWorkCallbackEpilog, 0, &TppWorkCancelPendingCallbacks};
...
static NTSTATUS __fastcall TppCleanupGroupMemberInitialize(
...
static NTSTATUS __fastcall TppWorkInitialize(
    _Inout_ _TP_WORK *pWork,
    _Inout_opt_ PVOID pv,
    _In_opt_ PTP_CALLBACK_ENVIRON pcbe,
    _In_ DWORD uFlags,
...
static void __fastcall TppCleanupGroupAddMember(_Outptr_ _TP_WORK *pWork)
{
...
static NTSTATUS __fastcall TpAllocWork(
    ...
    _In_opt_ PTP_CALLBACK_ENVIRON pcbe)
{
    ...
    auto Status = TppWorkInitialize(
        pWork,
        pv,
        pcbe,
        pcbe ? pcbe->u.Flags : 0,
        GetTppWorkpCleanupGroupMemberVFuncs(),
        GetTppWorkpTaskVFuncs());
...
{
    static const _TP_CLEANUP_GROUP_FUNCS TppWorkpCleanupGroupMemberVFuncs = {
        &TppWaitpFree, &TppWorkCallbackEpilog, 0, &TppWorkCancelPendingCallbacks};
    return &TppWorkpCleanupGroupMemberVFuncs;
}
...
static NTSTATUS __fastcall TpSimpleTryPost(
    ...
    _In_opt_ PTP_CALLBACK_ENVIRON CallbackEnviron)
{
    ...
    auto Status = TppWorkInitialize(
        ...
        CallbackEnviron,
        Flags,
        GetTppSimplepCleanupGroupMemberVFuncs(),
        GetTppSimplepTaskVFuncs());
...
static bool __fastcall TppCleanupGroupMemberRelease(
...
static void WINAPI TpReleaseWork(_TP_WORK *Work)
{
    if (Work == nullptr || (Work->uFlags1 & 0x10000) ||
        Work->VFuncs != Fallback::GetTppWorkpCleanupGroupMemberVFuncs())
    {
        internal::RaiseStatus(STATUS_INVALID_PARAMETER);
        return;
    }
...
static NTSTATUS __fastcall TppTimerAlloc(
    _TP_TIMER **ppTimer,
    ...
    DWORD Flags)
{
    ...
    *ppTimer = nullptr;
    ...
    auto Status = TppWorkInitialize(
        ...
        CallbackEnviron,
        Flags | 0x1040000,
        GetTppTimerpCleanupGroupMemberVFuncs(),
        GetTppTimerpTaskVFuncs());
...
static NTSTATUS __fastcall TpAllocTimer(
    _TP_TIMER **ppTimer,
    _In_ PTP_TIMER_CALLBACK Callback,
    _Inout_opt_ PVOID Context,
    _In_opt_ PTP_CALLBACK_ENVIRON CallbackEnviron)
...
// Minimum supported client	Windows Vista [desktop apps | UWP apps]
// Minimum supported server	Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, CloseThreadpoolWork, _Inout_ PTP_WORK pwk)
{
...
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, SubmitThreadpoolWork, _Inout_ PTP_WORK pwk)
{
    ...
    // 参数验证
    if (pwk == nullptr || (pwk->uFlags1 & 0x30000) ||
    ...
    _In_opt_ DWORD msWindowLength)
{
    ...
    if (pti == nullptr || (pti->uFlags1 & 0x30000) ||
    ...
    HANDLE hQueueTimer;
    auto bRet = CreateTimerQueueTimer(
        &hQueueTimer,
        nullptr,
        [](PVOID Parameter, BOOLEAN) {
    ...
    _In_ BOOL CancelPendingCallbacks)
{
    ...
    if (Timer == nullptr || (Timer->uFlags1 & 0x30000) ||
...
// Minimum supported client	Windows Vista [desktop apps | UWP apps]
// Minimum supported server	Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, CloseThreadpoolTimer, _Inout_ PTP_TIMER Timer)
{
    ...
    if (Timer == nullptr || (Timer->uFlags1 & 0x10000) ||
        Timer->VFuncs != Fallback::GetTppTimerpCleanupGroupMemberVFuncs())
    {
        internal::RaiseStatus(STATUS_INVALID_PARAMETER);
        return;
    ...
    _In_ HANDLE Event)
{
    ...
    // 只允许被设置一次
    if (Instance == nullptr || Event == nullptr || Event == INVALID_HANDLE_VALUE ||
...
{
    if (auto const pReleaseSemaphoreWhenCallbackReturns =
            wp_get_ReleaseSemaphoreWhenCallbackReturns())
    {
        return pReleaseSemaphoreWhenCallbackReturns(Instance, Semaphore, ReleaseCount);
    }

    // 只允许被设置一次
    if (Instance == nullptr || Semaphore == nullptr || Semaphore == INVALID_HANDLE_VALUE ||
        ReleaseCount == 0 || Instance->Semaphore != nullptr)
    {
        internal::RaiseStatus(STATUS_INVALID_PARAMETER);
        return;
    }
    ...
    _In_ HANDLE Mutex)
{
    ...
    // 只允许被设置一次
    if (Instance == nullptr || Mutex == nullptr || Mutex == INVALID_HANDLE_VALUE ||
...
{
    if (auto const pLeaveCriticalSectionWhenCallbackReturns =
            wp_get_LeaveCriticalSectionWhenCallbackReturns())
    {
        return pLeaveCriticalSectionWhenCallbackReturns(Instance, CriticalSection);
    }

    // 只允许被设置一次
    if (Instance == nullptr || CriticalSection == nullptr || Instance->CriticalSection != nullptr)
    {
        internal::RaiseStatus(STATUS_INVALID_PARAMETER);
        return;
    }
    ...
    _In_ HMODULE DllHandle)
{
    ...
    // 只允许被设置一次
    if (Instance == nullptr || DllHandle == nullptr || DllHandle == INVALID_HANDLE_VALUE ||
    ...
    _In_opt_ PTP_CALLBACK_ENVIRON CallbackEnviron)
{
    ...
    do
    {
        ...
        Status = Fallback::TppWorkInitialize(
            pWait,
            Context,
            CallbackEnviron,
            Flags | 0x40000,
            Fallback::GetTppWaitpCleanupGroupMemberVFuncs(),
            Fallback::GetTppWaitpTaskVFuncs());
...
// Minimum supported client	Windows Vista [desktop apps | UWP apps]
// Minimum supported server	Windows Server 2008 [desktop apps | UWP apps]
__DEFINE_THUNK(kernel32, 4, VOID, WINAPI, CloseThreadpoolWait, _Inout_ PTP_WAIT Wait)
{
    ...
    if (Wait == nullptr || (Wait->uFlags1 & 0x10000) ||
    ...
    _In_opt_ PFILETIME Timeout)
{
    ...
    if (Wait == nullptr || (Wait->uFlags1 & 0x30000) ||
        Wait->VFuncs != Fallback::GetTppWaitpCleanupGroupMemberVFuncs())
    {
        internal::RaiseStatus(STATUS_INVALID_PARAMETER);
        return;
    ...
    auto bRet = RegisterWaitForSingleObject(
        ...
        [](PVOID lpParameter, BOOLEAN TimerOrWaitFired) {
            ...
            // Timer->pTaskVFuncs->pExecuteCallback(&Instance, Timer);
            // 暂时这样硬编码……
            ((PTP_WAIT_CALLBACK)Wait->Callback)(
                &Instance, Wait->Context, Wait, TimerOrWaitFired ? WAIT_TIMEOUT : WAIT_OBJECT_0);
