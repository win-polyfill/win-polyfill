...
// 尝试锁定
static bool __fastcall TryAddSharedLock(FlsIndex *pIndex)
{
...
static void __fastcall ReleaseSharedLock(FlsIndex *pIndex)
{
    ...
    if (RawValue == (8 | FlsStatusUsing | FlsStatusAutoFree))
    {
        ...
        auto pFlsDataArray =
            (FlsData *)InterlockedExchangePointer((PVOID *)&pIndex->pFlsDataArray, nullptr);
        ...
        if (pFlsDataArray)
        {
            if (lpCallback)
            {
                for (auto pItem = pFlsDataArray, pItemEnd = pItem + FlsIndex::uFlsDataCount;
                     pItem != pItemEnd;
                     ++pItem)
                {
...
static FlsData *GetFlsData(FlsData *pFlsDataArray, bool bCanAlloc = false)
{
    ...
    if (bCanAlloc)
    {
        for (int i = 0; i != 128; ++i)
        {
            // 尝试搜索一个空闲的位置
            auto dwOrgThreadId =
                InterlockedCompareExchange(&pFlsDataGroup[i].dwThreadId, dwCurrentThreadId, 0);
...
static VOID NTAPI FlsDownlevelCallback(PVOID DllHandle, DWORD Reason, PVOID Reserved)
{
        ...
        for (auto &Item : Downlevel::GetFlsIndexArray())
        {
            ...
            if (internal::Block([&]() {
                    ...
                    for (;;)
                    {
                        ...
                        if ((CurrnetValue & (FlsStatusUsing | FlsStatusInit | FlsStatusAutoFree)) !=
                            FlsStatusUsing)
                            return false;
                        ...
                        auto OrgRawValue = InterlockedCompareExchange(
                            (unsigned long long *)&Item.RawValue, CurrnetValue + 8ll, CurrnetValue);
...
__DEFINE_THUNK(kernel32, 4, DWORD, WINAPI, FlsAlloc, _In_opt_ PFLS_CALLBACK_FUNCTION lpCallback)
{
    ...
    __declspec(allocate(".CRT$XLB")) static PIMAGE_TLS_CALLBACK __FLS_DOWNLEVEL_CALLBACK =
        Downlevel::FlsDownlevelCallback;
    ...
    for (auto &Item : FlsIndexArray)
    {
        ...
        for (;;)
        {
            ...
            // 尝试锁定Index
            auto OrgRawValue = InterlockedCompareExchange(
                &Item.RawValue, Downlevel::FlsStatusUsing | Downlevel::FlsStatusInit, RawValue);
            ...
            // 锁定成功
            auto pFlsDataArray = (Downlevel::FlsData *)HeapAlloc(
                ProcessHeap,
                HEAP_ZERO_MEMORY,
                sizeof(Downlevel::FlsData) * Downlevel::FlsIndex::uFlsDataCount);
...
__DEFINE_THUNK(kernel32, 4, BOOL, WINAPI, FlsFree, _In_ DWORD dwFlsIndex)
{
    ...
    do
    {
        ...
        if (internal::Block([&]() {
                ...
                {
                    if ((RawValue & (Downlevel::FlsStatusUsing | Downlevel::FlsStatusInit |
                                     Downlevel::FlsStatusAutoFree)) != Downlevel::FlsStatusUsing)
                    {
                        return false;
                    }
                    ...
                    auto OrgRawValue = InterlockedCompareExchange(
                        &Item.RawValue, RawValue + (8 | Downlevel::FlsStatusAutoFree), RawValue);
...
__DEFINE_THUNK(kernel32, 0, BOOL, WINAPI, IsThreadAFiber, VOID)
{
