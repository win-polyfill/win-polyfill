...
// Vista以后的Shell32.dll 有个 kfapi::GetFolderIdByCSIDL，我们可以根据它可以反推出出
// 下面的KnownFoldersIdToCSIDL
static int __fastcall KnownFoldersIdToCSIDL(const GUID &rfid)
{
    ...
    constexpr static const KnownFoldersIdKey KnownFoldersIdsMap[] = {
        ...
        {FOLDERID_ProgramFilesCommonX64,
         CSIDL_PROGRAM_FILES_COMMON}, // 兼容下，反正 x64路径 跟 普通的是一样的。
#endif
...
static __forceinline HRESULT __fastcall SHILCloneParent(
...
static HRESULT STDAPICALLTYPE SHBindToObject(
    ...
    _Outptr_ void **ppv)
{
    *ppv = nullptr;
    ...
    auto hr = psf ? psf->QueryInterface(__uuidof(IShellFolder), (void **)&ppshf)
...
static HRESULT STDAPICALLTYPE SHBindToFolderIDListParentEx(
...
static HRESULT __fastcall CShellItem_CreateInstance(PCWSTR pszCLSID, const IID &riid, void **ppv)
{
    *ppv = nullptr;
    ...
    auto hr =
        SHCoCreateInstance(nullptr, &CLSID_ShellItem, nullptr, __uuidof(IShellItem), (void **)&psi);
...
SHCreateItemFromIDList(_In_ PCIDLIST_ABSOLUTE pidl, _In_ REFIID riid, _Outptr_ void **ppv)
{
    *ppv = nullptr;
...
__forceinline static HRESULT ParseDisplayNameChild(
    ...
    /* [unique][out][in] */ __RPC__inout_opt ULONG *pdwAttributes)
{
    *ppidl = nullptr;
    ...
    auto hr =
        pShellFolder->ParseDisplayName(hwnd, pbc, pszDisplayName, pchEaten, &pidl, pdwAttributes);
...
static HRESULT STDAPICALLTYPE SHCreateItemWithParent(
...
static HRESULT STDAPICALLTYPE SHCreateItemFromRelativeName(
    ...
    _Outptr_ void **ppv)
{
    *ppv = nullptr;
    ...
    auto hr = psiParent->BindToHandler(
        nullptr, BHID_SFObject, __uuidof(pShellFolder), (void **)&pShellFolder);

    if (SUCCEEDED(hr))
    {
        PIDLIST_RELATIVE pidl;

        hr = internal::ParseDisplayNameChild(
            pShellFolder, nullptr, pbc, (LPWSTR)pszName, nullptr, &pidl, nullptr);
    ...
    _Outptr_ IShellItem **ppsi)
{
    ...
    *ppsi = nullptr;
    ...
    if (pidlParent || psfParent)
        return internal::SHCreateItemWithParent(
            pidlParent, psfParent, pidl, __uuidof(IShellItem), (void **)ppsi);
    else
...
__DEFINE_THUNK(
    ...
    _Outptr_ void **ppv)
{
    if (const auto pSHCreateItemFromParsingName = wp_get_SHCreateItemFromParsingName())
    {
