...
{
    return szStr[0] == L'{' && IsHexDigitWorker(szStr[1]) && IsHexDigitWorker(szStr[2]) &&
           IsHexDigitWorker(szStr[3]) && IsHexDigitWorker(szStr[4]) && IsHexDigitWorker(szStr[5]) &&
           IsHexDigitWorker(szStr[6]) && IsHexDigitWorker(szStr[7]) && IsHexDigitWorker(szStr[8]) &&
           szStr[9] == L'-' && IsHexDigitWorker(szStr[10]) && IsHexDigitWorker(szStr[11]) &&
           IsHexDigitWorker(szStr[12]) && IsHexDigitWorker(szStr[13]) && szStr[14] == L'-' &&
           IsHexDigitWorker(szStr[15]) && IsHexDigitWorker(szStr[16]) &&
           IsHexDigitWorker(szStr[17]) && IsHexDigitWorker(szStr[18]) && szStr[19] == L'-' &&
           IsHexDigitWorker(szStr[20]) && IsHexDigitWorker(szStr[21]) &&
           IsHexDigitWorker(szStr[22]) && IsHexDigitWorker(szStr[23]) && szStr[24] == L'-' &&
           IsHexDigitWorker(szStr[25]) && IsHexDigitWorker(szStr[26]) &&
           IsHexDigitWorker(szStr[27]) && IsHexDigitWorker(szStr[28]) &&
           IsHexDigitWorker(szStr[29]) && IsHexDigitWorker(szStr[30]) &&
           IsHexDigitWorker(szStr[31]) && IsHexDigitWorker(szStr[32]) &&
           IsHexDigitWorker(szStr[33]) && IsHexDigitWorker(szStr[34]) &&
           IsHexDigitWorker(szStr[35]) && IsHexDigitWorker(szStr[36]) && szStr[37] == L'}';
}
...
static bool __fastcall PathIsVolumeGUIDWorker(LPCWSTR pszPath)
{
    return pszPath[0] == L'\\' && pszPath[1] == L'\\' && pszPath[2] == L'?' &&
           ...
           (pszPath[9] == L'E' || pszPath[9] == L'e') && internal::StringIsGUIDWorker(pszPath + 10);
}
...
{
    return pszPath[0] == L'\\' && pszPath[1] == L'\\' && pszPath[2] == L'?' && pszPath[3] == L'\\';
}
...
// Minimum supported client	Windows 8 [desktop apps | UWP apps]
// Minimum supported server	Windows Server 2012 [desktop apps | UWP apps]
__DEFINE_THUNK(api_ms_win_core_path_l1_1_0, 4, BOOL, WINAPI, PathCchIsRoot, _In_opt_ PCWSTR pszPath)
{
    // NULL or ""
     ...
     */
    PCWSTR pszServer;
    if (Thunks::PathIsUNCEx(pszPath, &pszServer))
    {
    ...
    *ppszRootEnd = nullptr;
    ...
    else
    {
        if (internal::PathIsVolumeGUIDWorker(pszPath))
        {
            ...
            pszPath[0] == L'\\' && pszPath[1] == L'\\' && pszPath[2] == L'?' &&
            pszPath[3] == L'\\' &&
    ...
    _In_ ULONG dwFlags)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (internal::IsExtendedLengthDosDevicePath(pszPathIn))
            {
                if ((L'A' <= pszPathIn[4] && pszPathIn[4] <= L'Z') ||
                    (L'a' <= pszPathIn[4] && pszPathIn[4] <= L'z') && pszPathIn[5] == L':')
                {
                    pszServer += 4;
                ...
                (L'a' <= pszPathIn[0] && pszPathIn[0] <= L'z') && pszPathIn[1] == L':')
            {
                bSupportDosDevicePath = true;
            }
        ...
        for (; *pszServer;)
        {
            ...
            else if (cchFileName == 2 && pszServer[0] == L'.' && pszServer[1] == L'.')
            {
                // 表示上级目录
                ...
                {
                    auto szPrevious =
                        (wchar_t *)internal::FindPreviousBackslashWorker(pszPathOut, pTempOut - 1);
                ...
                if (pTempOut + cchFileName >= pszPathOutEnd)
                {
                    ...
                    if (cchFileName == 1 && pszServer[0] == L'\\')
                    {
                            ...
                            (pszServer[1] == L'.' && pszServer[2] == L'\0'))
                        {
                            ...
                            pTempOut + 1 == pszPathOutEnd && pszServer[1] == L'.' &&
                            pszServer[2] == L'.')
                        {
                            // 下一个项目是返回上一个项目，所以可以伪造一下，直接设置为 L'\0'
                            // 然后往下移动
                            ++pszServer;
                            *pTempOut++ = L'\0';
                            continue;
        ...
        if (pszPathOut + nCountOfData <= pTempOut && pTempOut[-7] == L':' && pTempOut[-6] == L':' &&
            pTempOut[-5] == L'$' && (pTempOut[-4] == L'D' || pTempOut[-3] == L'd') &&
...
__DEFINE_THUNK(
    ...
    _In_ ULONG dwFlags)
{
    if (pszPathOut == nullptr || cchPathOut == 0 || cchPathOut > PATHCCH_MAX_CCH)
    {
        return E_INVALIDARG;
    }
    ...
    _In_ size_t cchPath)
{
    ...
    if ((pszPath[4] == L'U' || pszPath[4] == L'u') && (pszPath[5] == L'N' || pszPath[5] == L'n') &&
        (pszPath[6] == L'C' || pszPath[6] == L'c') && pszPath[7] == L'\\')
    {
        if (cchPath < 8)
            return E_INVALIDARG;
        ...
        (L'A' <= pszPath[4] && pszPath[4] <= L'Z' || L'a' <= pszPath[4] && pszPath[4] <= L'z') &&
        pszPath[5] == L':')
    {
