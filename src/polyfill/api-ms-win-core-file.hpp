    ...
    _In_ DWORD dwBufferSize)
{
    ...
    else
    {
        ...
        Status = pNtQueryInformationFile(
            hFile, &IoStatusBlock, lpFileInformation, dwBufferSize, NtFileInformationClass);
    }
    ...
    _In_ DWORD dwBufferSize)
{
    ...
    auto Status = pNtSetInformationFile(
        hFile, &IoStatusBlock, lpFileInformation, dwBufferSize, NtFileInformationClass);
    ...
    _In_ DWORD dwFlags)
{
    ...
    if (nullptr == pNtQueryObject || nullptr == pNtQueryInformationFile)
    {
        SetLastError(ERROR_INVALID_FUNCTION);
        return 0;
    }
    ...
    for (;;)
    {
        ...
        auto Status =
            pNtQueryObject(hFile, ObjectNameInformation, pObjectName, cbObjectName, &cbObjectName);
    ...
    for (;;)
    {
        ...
        auto Status = pNtQueryInformationFile(
            hFile, &IoStatusBlock, pFileNameInfo, cbFileNameInfo, FileNameInformation);
    ...
    VolumeNtName.Length = VolumeNtName.MaximumLength =
        pObjectName->Name.Length - pFileNameInfo->FileNameLength + sizeof(wchar_t);
    ...
    if ((FILE_NAME_OPENED & dwFlags) == 0)
    {
        ...
        szLongPathNameBuffer[(cbszVolumeRoot + pFileNameInfo->FileNameLength) / sizeof(wchar_t)] =
            L'\0';
        ...
        for (;;)
        {
            ...
            else if (result >= cchLongPathNameBufferSize)
            {
                ...
                auto pNewLongPathName = (wchar_t *)HeapReAlloc(
                    ProcessHeap,
                    0,
                    szLongPathNameBuffer,
                    cchLongPathNameBufferSize * sizeof(wchar_t));
                if (!pNewLongPathName)
                {
                    lStatus = ERROR_NOT_ENOUGH_MEMORY;
                    goto __Exit;
    ...
    _In_ DWORD dwFlags)
{
    ...
    for (DWORD cchszFilePathUnicode = 1040;;)
    {
        ...
        {
            auto pNewBuffer = (wchar_t *)HeapReAlloc(
                ProcessHeap, 0, szFilePathUnicode, cchszFilePathUnicode * sizeof(wchar_t));
            if (!pNewBuffer)
            {
        ...
        {
            szFilePathUnicode =
                (wchar_t *)HeapAlloc(ProcessHeap, 0, cchszFilePathUnicode * sizeof(wchar_t));
            if (!szFilePathUnicode)
            {
                SetLastError(ERROR_NOT_ENOUGH_MEMORY);
                return 0;
    ...
    _In_ DWORD dwFlagsAndAttributes)
{
    ...
    OBJECT_ATTRIBUTES ObjectAttributes = {
        sizeof(ObjectAttributes), hVolumeHint, &ObjectName, OBJ_CASE_INSENSITIVE};
    ...
    do
    {
        ...
        if (FILE_FLAG_OPEN_NO_RECALL & dwFlagsAndAttributes)
        {
            SecurityQualityOfService.ImpersonationLevel =
                SECURITY_IMPERSONATION_LEVEL((dwFlagsAndAttributes >> 16) & 0x3);
