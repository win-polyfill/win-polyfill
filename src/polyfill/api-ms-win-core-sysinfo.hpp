...
__DEFINE_THUNK(kernel32, 0, ULONGLONG, WINAPI, GetTickCount64, VOID)
{
    ...
    _Out_ PDWORD pdwReturnedProductType)
{
    ...
    if (GetVersionExW((LPOSVERSIONINFOW)&VersionInfo))
    {
        if (VersionInfo.wSuiteMask & VER_SUITE_ENTERPRISE)
        {
            ...
            VER_SUITE_SMALLBUSINESS_RESTRICTED) // VER_SUITE_SMALLBUSINESS 与
                                                // VER_SUITE_SMALLBUSINESS_RESTRICTED
        {
            ...
            VersionInfo.wSuiteMask &
            VER_SUITE_EMBEDDED_RESTRICTED) // VER_SUITE_EMBEDDEDNT 与 VER_SUITE_EMBEDDED_RESTRICTED
        {
        ...
        else
        {
            if (VersionInfo.wProductType == VER_NT_WORKSTATION)
            {
                ...
                VersionInfo.wProductType == VER_NT_SERVER)
            {
                // 如果目标系统是5.1、5.2，因为SuiteMask中检测了Small Business Server、Web
                // Edition、Enterprise Edition、 Datacenter Edition、Compute Cluster Server、Storage
                // Server，所以这里只可能是Standard Edition
                //
                // 如果目标系统是5.0，因为SuiteMask中检测了Datacenter Server，这里只能是 Advanced
                // Server或者 Server，但是我没有找到 Advanced，姑且用 PRODUCT_STANDARD_SERVER
                // 凑合吧。
